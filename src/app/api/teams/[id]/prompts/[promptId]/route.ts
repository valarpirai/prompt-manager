...
export async function DELETE(req: NextRequest, { params }: { params: { id: string; promptId: string } }) {
  return withAuth(async (authReq: AuthenticatedRequest) => {
    try {
      ...
      if (isNaN(teamId) || isNaN(promptId)) {
        return NextResponse.json({ error: 'Invalid team or prompt ID' }, { status: 400 })
      }
      ...
      if (!prompt) {
        return NextResponse.json({ error: 'Prompt not found in this team' }, { status: 404 })
      }
