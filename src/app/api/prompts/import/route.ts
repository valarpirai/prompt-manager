...
export const POST = withAuth(async (req: AuthenticatedRequest) => {
  try {
    ...
    const textIndex = headers.findIndex(
      (h) => h === 'text' || h === 'prompt_text' || h === 'prompt text'
    );

    if (nameIndex === -1) {
      return NextResponse.json(
        { error: 'CSV must contain a "Name" or "Title" column' },
        { status: 400 }
      );
    }
    ...
    if (textIndex === -1) {
      return NextResponse.json(
        {
          error:
            'CSV must contain a "Text", "Prompt_Text", or "Prompt Text" column',
        },
        { status: 400 }
      );
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        if (existingPrompt) {
          results.errors.push(
            `Row ${i + 1}: Prompt with title "${title}" already exists`
          );
          results.skipped++;
          continue;
        }
