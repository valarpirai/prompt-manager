...
export const GET = withAuthAndCors(async (req: AuthenticatedRequest) => {
  try {
    ...
    if (filterType === 'all') {
      ...
      where.OR = [{ visibility: 'PUBLIC' }, { owner_id: req.user!.userId }];
      ...
      where.AND = [{ visibility: 'PRIVATE' }, { owner_id: req.user!.userId }];
  ...
  try {
    ...
    if (teamId) {
      ...
      if (!teamMember) {
        return NextResponse.json(
          {
            error: 'You do not have permission to create prompts for this team',
          },
          { status: 403 }
        );
      }
