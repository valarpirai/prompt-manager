...
export function withAuthAndCors(handler: (req: AuthenticatedRequest) => Promise<NextResponse>) {
  return async (req: NextRequest) => {
    ...
    if (!payload.isVerified) {
      const errorResponse = NextResponse.json({ error: 'Email not verified' }, { status: 403 })
      return addCorsHeaders(errorResponse)
    }
